...
test_that("Warning when min and max not specified ut clean run 1",{
  ...
  expect_equivalent(test1$X1, c(9, 5, 4, 8, 4, 5, 3, 1, 6, 5))
  expect_equivalent(test1$new, c(1.000, 0.500, 0.375, 0.875, 0.375, 0.500, 0.250, 0.000, 0.625, 0.500))
  on.exit(rm(test1))
})
...
test_that("clean run 2",{
  ...
  expect_equivalent(test2$X2, c(9, 5, 3, 7, 3, 3, 4, 7, 3, 7))
  expect_equivalent(test2$new, c(100.0, 34.0, 1.0, 67.0, 1.0, 1.0, 17.5, 67.0, 1.0, 67.0))
  on.exit(rm(test2))
})
...
test_that("clean run 3",{
  ...
  expect_equivalent(test3$X3, c(2, 3, 9, 4, 1, 7, 10, 3, 2, 6))
  expect_equivalent(test3$new, c(1.7, 2.3, 6.3, 3.0, 1.0, 5.0, 7.0, 2.3, 1.7, 4.3))
  on.exit(rm(test3))
})
...
test_that("clean run 4",{
  ...
  expect_equivalent(test4$X1, c(9, 5, 4, 8, 4, 5, 3, 1, 6, 5))
  expect_equivalent(test4$new, c(1, 5, 6, 2, 6, 5, 7, 9, 4, 5))
  on.exit(rm(test4))
})
